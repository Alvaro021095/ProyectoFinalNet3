...
function guardar() {
    ...
    if (nombre != "" && descripcion != "" && direccion != "") {
        $.ajax({
            type: 'POST',
            url: "/Bodega/SaveInfo",
            data: { id: id, nombre: nombre,direccion: direccion ,descripcion: descripcion },
...
function buscar() {
    ...
    if (nombre != "") {
        ...
        $.ajax({
            type: 'POST',
            url: "/Bodega/SearchInfo",
            data: { nombre: nombre },
            success: function (data) {
                if (data.d.length > 0) {
                    $("#txtId").val(data.d[0]);
                    $("#txtNombre").val(data.d[1]);
                    $("#txtDireccion").val(data.d[2]);
                    $("#txtDescripcion").val(data.d[3]);
                } else {
